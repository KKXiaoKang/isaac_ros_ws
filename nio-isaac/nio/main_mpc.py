...
class KuavoController:
    def __init__(self):
        ...
        # 等待MPC服务可用
        self.enable_mpc_service = rospy.ServiceProxy('/humanoid_controller/enable_mpc', Trigger)
    ...
    def get_cmd(self, obs, sensor_data=None):
        """
    ...
    def check_and_enable_mpc(self):
        ...
        if elapsed_time >= MPC_ENABLE_TIME:
            try:
                # 调用服务启用MPC
                response = self.enable_mpc_service()
                if response.success:
                    rospy.loginfo("MPC enabled successfully: %s", response.message)
                    self.mpc_enabled = True
                else:
                    rospy.logwarn("Failed to enable MPC: %s", response.message)
...
def main():
    ...
    while True:
        ...
        # 检查是否需要启用MPC
        ctrl.check_and_enable_mpc()

        # 获取传感器数据
        sensor_data = sensor.get_obs()
        
        # 将sensor_data传递给get_cmd
        action = ctrl.get_cmd(kuavo.get_obs(), sensor_data)
        
        # 检查是否收到第一个命令
        if not first_cmd_received and action is not None:
            print("First command received. Starting simulation...")
            env.sim.play()
        ...
        imu_data = sensor_data["imu"]
        # print("\nIMU Data:")
        # print(f"Time: {imu_data['time']:.3f}")
        # print(f"Physics Step: {imu_data['physics_step']}")
